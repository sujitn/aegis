...
impl NsfwImageClassifier {
    ...
    pub fn classify_bytes(&mut self, image_data: &[u8]) -> Result<NsfwImageResult, NsfwImageError> {
        ...
        // Channel offsets for CHW format
        ...
        for (y, row) in rgb_img.rows().enumerate() {
            for (x, pixel) in row.enumerate() {
                ...
                let pixel_offset = y * input_size + x;
                tensor_data[r_offset + pixel_offset] = r;
                tensor_data[g_offset + pixel_offset] = g;
                tensor_data[b_offset + pixel_offset] = b;
            }
        }
